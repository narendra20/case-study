...
def produce_triples(reviews_file, menus_file, review_filter=None, restaurant_filter=None, verbose=False):
    ...
    for review in reviews:
        ...
        rtext = re.sub(r'[!,.:;-](?=|$)',r' ',review['review_text']) 
        rid = review['restaurant_id']
        rrating = review['review_rating']
        rdate = review['review_date']
        username = review['user_name']
        menu = menus_by_restaurant[rid]
        for menu_item, text in look_for_menu_item(menu, rtext):
            yield rid, menu_item, text, rrating, rdate, username
    sys.stderr.write("\r1.0\n")


def main():
    reviews_file = "/Users/anishumesh/Downloads/Picking-dishes-from-Yelp-master/yelp-dataset/reviews.json"
    menus_file = "/Users/anishumesh/Downloads/Picking-dishes-from-Yelp-master/yelp-dataset/menus.json"

    produce_triples(reviews_file, menus_file)

    f = open('triples.csv', 'w')
    f.write('restaurant_id,menu_item,text,review_rating,review_date,user_name\n')
    count = 0
    print("Working on it...Please wait...")
    start = time.time()
    for restaurant, menu_item, review_text, review_rating, review_date, user_name in produce_triples(reviews_file, menus_file):
        f.write('{},{},{},{},{},{}\n'.format(restaurant, menu_item, review_text, review_rating, review_date, user_name))
        count += 1
        print([restaurant, menu_item, review_text, review_text, review_rating, review_date, user_name])
    f.close()
